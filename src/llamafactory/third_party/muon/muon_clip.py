    ...
    def _apply_qk_clip(self):
        ...
        for layer_name, attention_layer in self.attention_layers:
            ...
            if max_logits > tau:
                ...
                with torch.no_grad():
                    # 1. Content 部分 (q, k) -> sqrt(gamma)
                    layer.w_dq.weight.data *= sqrt_gamma  # Down-project query
                    layer.w_uk.weight.data *= sqrt_gamma  # Up-project key (content)
                    
                    # 2. RoPE Query 部分 -> gamma (注意这里是 gamma)
                    # 论文中提到 qR (head-specific rotary) scaled by gamma
                    layer.w_qr.weight.data *= gamma 
                    
                    # 3. RoPE Shared Key -> 不动
                    # layer.w_kr (Shared rotary) touches nothing
...
#with no grad??
self.max_logits = attn_weights.max().detach().item()
